...
const bookAppointmentCallback = async (queary: Record<string, any>) => {
	...
	if (status === "success") {
		...
		pdfDocument
			.fontSize(12)
			.text(`Patient Name: ${transactionResult.appointment.patient?.name}`);
		pdfDocument.text(`Patient Email: ${transactionResult.appointment.patient?.email}`);
		pdfDocument.moveDown(2);

		pdfDocument.text(`Doctor Name: ${transactionResult.appointment.doctor?.name}`);
		pdfDocument.text(
			`Specialization: ${transactionResult.appointment.doctor?.specialization}`,
		);
		pdfDocument.moveDown();
		pdfDocument.text(
			`Appointment Date:${transactionResult.appointment.schedule.startDateTime.toDateString()}`,
		);
		pdfDocument.text(
			`Your Joining Time:${transactionResult.joiningTime.toString()}`,
		);
		pdfDocument.text(`Your Serial Number:${transactionResult.serialNumber}`);
		pdfDocument.text(
			`Meeting Link:${transactionResult.appointment.schedule.meetingLink}`,
		);
		pdfDocument.moveDown();
		...
		await transporter.sendMail({
			...
			to: transactionResult.appointment.patient.email,
