...
import type { IPostQuery } from "../../interface";
...
const createSchedule = async (
	...
	if (!isSameDay(payload.startDateTime, payload.endDateTime)) {
		throw new AppError(
			httpStatus.CONFLICT,
			"Start Date Time And End Date Time Muar Be On The same Day",
		);
	}
...
const updateSchedule = async (
	...
	if (
		...
		throw new AppError(
			httpStatus.CONFLICT,
			"schedule Once Published cannot Be Updated",
		);
	}
...
const getTodaysSchedul = async (query : IPostQuery) => {
    ...
    const andConditions: ScheduleWhereInput[] = [
        ...
        {
			startDateTime : { gte : startOfToday, lt : startOfTomorrow },
		},
		{
			endDateTime: { gt: now },
        },
